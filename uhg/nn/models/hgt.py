...
class HyperbolicTransformerLayer(nn.Module):
    ...
    def forward(
        ...
        x_features = torch.cat(
            [pre_features[..., :2], x_features_norm[..., 2:]], dim=-1
        )
        ...
        x_features = torch.cat(
            [pre_features2[..., :2], x_features_norm2[..., 2:]], dim=-1
        )
        x = torch.cat([x_features, x[..., -1:]], dim=-1)
...
class HGT(nn.Module):
    ...
    def __init__(
        ...
        self.output_proj = HyperbolicLinear(
            manifold=self.manifold, in_features=d_model, out_features=d_model, bias=True
        )
