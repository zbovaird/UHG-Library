...
class UHGLoss(nn.Module):
    ...
    def forward(
        self, z: torch.Tensor, edge_index: torch.Tensor, batch_size: int
    ) -> torch.Tensor:
        ...
        pos_quad = torch.clamp(
            uhg_quadrance(z[pos_edge_index[0]], z[pos_edge_index[1]]), max=10.0
        )

        # Generate and compute negative pairs
        neg_edge_index = torch.randint(0, batch_size, (2, batch_size), device=z.device)
        neg_quad = torch.clamp(
            uhg_quadrance(z[neg_edge_index[0]], z[neg_edge_index[1]]), max=10.0
        )

        # Compute spread for geometric structure preservation
        spread = torch.clamp(
            uhg_spread(z[pos_edge_index[0]], z[pos_edge_index[1]]), max=10.0
        )
...
class UHGAnomalyLoss(UHGLoss):
    ...
    def __init__(
        self, spread_weight: float = 0.1, quad_weight: float = 1.0, margin: float = 1.0
    ):
