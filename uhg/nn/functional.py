...
def uhg_weighted_midpoint(
    points: torch.Tensor, weights: torch.Tensor, eps: float = 1e-9
) -> torch.Tensor:
    """
    ...
    norm = torch.sqrt(
        torch.sum(weighted_sum**2, dim=-1, keepdim=True)
        - weighted_sum[..., -1:] ** 2
        + eps
    )
...
def uhg_attention_kernel(
    query: torch.Tensor, key: torch.Tensor, eps: float = 1e-9
) -> torch.Tensor:
    """
...
def scatter_mean_custom(
    src: torch.Tensor, index: torch.Tensor, dim: int = 0, dim_size: int = None
) -> torch.Tensor:
...
def uhg_normalize(x: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    norm = torch.sqrt(
        torch.clamp(
            torch.sum(features**2, dim=-1, keepdim=True) - homogeneous**2, min=eps
        )
    )
