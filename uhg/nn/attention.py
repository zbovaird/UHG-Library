...
class HyperbolicAttention(nn.Module):
    ...
    def __init__(
        ...
        self.concat = concat
        self.skip_final_proj = skip_final_proj
        ...
        # Output projection (HGAT may apply its own HyperbolicLinear instead)
        if skip_final_proj:
    ...
    def forward(
        ...
        if edge_attr is not None:
            if edge_attr.shape[-1] != self.in_features:
                if (
                    ...
                    or self.edge_mlp.in_features != edge_attr.shape[-1]
                ):
                    self.edge_mlp = nn.Linear(edge_attr.shape[-1], self.in_features).to(
                        edge_attr.device
                    )
            ...
            edge_proj_heads = edge_proj.unsqueeze(1).expand(
                -1, self.num_heads, -1
            )  # [E, num_heads, in_features]
        ...
        if edge_proj_heads is not None:
            value_messages = (
                value_messages + edge_proj_heads
            )  # [E, num_heads, out_features]

        # Multiply by attention weights
        value_messages = value_messages * alpha.unsqueeze(
            -1
        )  # [E, num_heads, out_features]
        ...
        out.scatter_add_(
            ...
            row.unsqueeze(-1)
            .unsqueeze(-1)
            .expand(-1, self.num_heads, self.out_features),
            value_messages,
        )
        ...
        # Output projection (Identity when skip_final_proj: HGAT applies HyperbolicLinear after)
