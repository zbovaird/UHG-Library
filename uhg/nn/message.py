...
class HyperbolicMessagePassing(nn.Module):
    ...
    def __init__(
        ...
        self.message_mlp = nn.Sequential(
            nn.Linear(
                in_features * 2 + (edge_features if edge_features else 0), out_features
            ),
            nn.ReLU(),
    ...
    def forward(
        ...
        if self.aggr == "mean":
            ...
            out.scatter_add_(
                0, row.unsqueeze(-1).expand(-1, self.out_features), messages
            )
            ...
            out.scatter_reduce_(
                0,
                row.unsqueeze(-1).expand(-1, self.out_features),
                messages,
                reduce="amax",
            )
        else:
