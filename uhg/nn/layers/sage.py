...
class ProjectiveSAGEConv(UHGLayer):
    ...
    def aggregate_neighbors(
        ...
        out = torch.zeros(out_size, self.out_features + 1, device=x.device, dtype=x.dtype)
        ...
        transformed = self.projective_transform(neigh_features, self.W_neigh).to(x.dtype)
        ...
        if transformed.size(-1) != target_dim:
            if transformed.size(-1) < target_dim:
                ...
                transformed = torch.cat([transformed[..., :-1], pad, transformed[..., -1:]], dim=-1)
            else:
        ...
        if self.aggregator == 'mean':
            ...
            fill_val = torch.finfo(spatial.dtype).min if spatial.is_floating_point() else -2**31
            out_spatial = spatial.new_full((out_size, spatial.size(-1)), fill_val)
            out_spatial.scatter_reduce_(0, row.unsqueeze(-1).expand(-1, spatial.size(-1)), spatial, reduce='amax')
            # Replace uninitialized nodes with zeros
            is_init = (out_spatial != fill_val).any(dim=-1, keepdim=True)
            out[..., :-1] = torch.where(is_init.expand_as(out[..., :-1]), out_spatial, torch.zeros_like(out_spatial))
    ...
    def forward(
        ...
        if self_trans.size(-1) != target_dim:
            if self_trans.size(-1) < target_dim:
                ...
                self_trans = torch.cat([self_trans[..., :-1], pad, self_trans[..., -1:]], dim=-1)
            else:
