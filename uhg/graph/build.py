...
def build_knn_graph(
    ...
    Uses Euclidean distance by default (NearestNeighbors). Edges are directed:
    (i, j) means j is a neighbor of i.
    ...
    Args:
        ...
        cache_key: If provided, save/load graph from cache. Auto-generated from
            (X.shape, k, metric) if None and caching is used.
    ...
    if cache_key is not None or cache_dir is not None:
        key = cache_key or _cache_key(X_np.shape, k, metric)
...
def _cache_key(shape: tuple, k: int, metric: str) -> str:
    h = hashlib.sha256(f"{shape}_{k}_{metric}".encode()).hexdigest()[:16]
