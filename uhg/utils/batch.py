...
class UHGBatch:
    ...
    def to_batch(self, data_list: List[Data], preserve_cr: bool = True) -> Batch:
    ...
    def unbatch(self, batch: Batch, restore_cr: bool = True) -> List[Data]:
        ...
        if restore_cr and hasattr(batch, "cross_ratios"):
            ...
            for i, (data, cr) in enumerate(zip(data_list, batch.cross_ratios)):
                if cr is not None and data.x.size(0) > 3:
                    ...
                    if (
                        not torch.isnan(cr_current)
                        and not torch.isnan(cr)
                        and cr_current != 0
                    ):
                        # Compute scale factor in log space
                        log_scale = 0.5 * (
                            torch.log(cr + 1e-8) - torch.log(cr_current + 1e-8)
                        )
