...
class ThreatCorrelation:
    ...
    def __init__(self, feature_dim: int = 8, num_heads: int = 4, dropout: float = 0.1):
    ...
    def correlate(
        ...
        correlations, weights = self.attention(
            query_indicators, proj_indicators, proj_indicators
        )

        return correlations.squeeze(0), weights.squeeze(0)

    def get_correlation_groups(
        self, indicators: List[ThreatIndicator], threshold: float = 0.7
    ...
    def analyze_indicator_relationships(
        ...
        for i in range(len(indicators)):
            for j in range(i + 1, len(indicators)):
                ...
                if (
                    indicators[i].type == ThreatIndicatorType.PAYLOAD
                    or indicators[j].type == ThreatIndicatorType.PAYLOAD
                ):
