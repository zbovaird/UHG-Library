...
class UHGMultiHeadAttention(nn.Module):
    ...
    def __init__(self, config: UHGAttentionConfig):
        ...
        q_init = torch.zeros(
            config.num_heads, config.feature_dim + 1, config.feature_dim + 1
        )
        k_init = torch.zeros(
            config.num_heads, config.feature_dim + 1, config.feature_dim + 1
        )
    ...
    def _get_ideal_points(
        self, line: torch.Tensor
    ...
    def _compute_attention_scores(
        ...
        scores = torch.zeros(
            batch_size, num_heads, num_queries, num_keys, device=q.device
        )
        ...
        if self.config.use_cross_ratio:
            ...
            for b in range(batch_size):
                for h in range(num_heads):
                    i1 = torch.zeros(q.size(-1), device=q.device)
                    i1[0] = 1.0
                    i1[-1] = 0.0
                    i2 = torch.zeros(q.size(-1), device=q.device)
                    i2[1] = 1.0
                    i2[-1] = 0.0
                    for i in range(num_queries):
                        for j in range(num_keys):
                            ...
                            scores[b, h, i, j] = 1.0 / (
                                1.0
                                + torch.abs(torch.log(torch.abs(cr) + self.config.eps))
                            )
        else:
