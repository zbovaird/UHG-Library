...
class UHGConv(nn.Module):
    ...
    def aggregate_messages(
        self, messages: torch.Tensor, row: torch.Tensor, num_nodes: int
    ) -> torch.Tensor:
        """
        ...
        out.scatter_add_(0, row.unsqueeze(-1).expand(-1, messages.size(-1)), messages)
...
class UHGMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        q = q.view(seq_len, batch_size, self.num_heads, self.head_dim).permute(
            0, 2, 1, 3
        )  # (seq_len, num_heads, batch_size, head_dim)
        k = k.view(seq_len, batch_size, self.num_heads, self.head_dim).permute(
            0, 2, 1, 3
        )  # (seq_len, num_heads, batch_size, head_dim)
        v = v.view(seq_len, batch_size, self.num_heads, self.head_dim).permute(
            0, 2, 1, 3
        )  # (seq_len, num_heads, batch_size, head_dim)
        ...
        scores = torch.zeros(
            seq_len, self.num_heads, batch_size, seq_len, device=q.device
        )
        for h in range(self.num_heads):
            for b in range(batch_size):
                for i in range(seq_len):
                    for j in range(seq_len):
                        scores[i, h, b, j] = -self.metric.distance(
                            q[i, h, b], k[j, h, b]
        ...
        output = torch.zeros(
            seq_len, self.num_heads, batch_size, self.head_dim, device=q.device
        )
        for h in range(self.num_heads):
            for b in range(batch_size):
                for i in range(seq_len):
                    output[i, h, b] = torch.sum(
                        attn_weights[i, h, b, :].unsqueeze(-1) * v[:, h, b, :], dim=0
                    )
        ...
        attn_weights_out = attn_weights_out.unsqueeze(2).expand(
            -1, -1, seq_len, -1
        )  # (seq_len, num_heads, seq_len, seq_len)
