...
class ProjectiveDataset(Dataset):
    ...
    def __init__(self, points: torch.Tensor, labels: Optional[torch.Tensor] = None):
    ...
    def compute_cross_ratios(
        self, idx1: int, idx2: int, idx3: int, idx4: int
    ) -> torch.Tensor:
    ...
    def get_neighborhood(self, idx: int, radius: float) -> List[int]:
        ...
        distances = torch.tensor([self.uhg.proj_dist(center, p) for p in self.points])
    ...
    def sample_triplet(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        pos_dists = torch.tensor([self.uhg.proj_dist(anchor, p) for p in self.points])
