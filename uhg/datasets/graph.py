...
class ProjectiveGraphDataset(ProjectiveDataset):
    ...
    def __getitem__(self, idx: int) -> Data:
        ...
        data = Data(
            x=self.points[idx],
            edge_index=(
                self.edge_index[idx] if self.edge_index.dim() > 2 else self.edge_index
            ),
    ...
    def to(self, device: torch.device) -> "ProjectiveGraphDataset":
        ...
        graph_labels = (
            self.graph_labels.to(device) if self.graph_labels is not None else None
        )

        return ProjectiveGraphDataset(
            node_features, edge_index, edge_attr, node_labels, graph_labels
        )

    def get_neighbors(
        self, node_idx: int, edge_index: Optional[torch.Tensor] = None
    ) -> List[int]:
    ...
    def compute_edge_cross_ratios(self) -> torch.Tensor:
        ...
        for s, d in zip(src, dst):
            ...
            # Compute cross-ratio
            cr = self.uhg.cross_ratio(self.points[s], self.points[d], i1, i2)
            cross_ratios.append(cr)

        return torch.tensor(cross_ratios)

    def get_subgraph(
        self, node_idx: int, n_hops: int = 1
        ...
        src, dst = self.edge_index
        mask = torch.tensor([s in nodes and d in nodes for s, d in zip(src, dst)])
    ...
    @classmethod
    def from_networkx(
        cls, G: "networkx.Graph", node_features: Optional[torch.Tensor] = None
