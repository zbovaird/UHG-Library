...
class UHGAdam(UHGBaseOptimizer):
    ...
    def _compute_hyperbolic_momentum(
        self, grad: torch.Tensor, param: torch.Tensor, beta: float, group: dict
    ) -> torch.Tensor:
        """
    ...
    def _compute_hyperbolic_second_moment(
        self, grad: torch.Tensor, param: torch.Tensor, beta: float, group: dict
    ) -> torch.Tensor:
        """
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                exp_avg_sq = self._compute_hyperbolic_second_moment(
                    grad, param, beta2, group
                )
