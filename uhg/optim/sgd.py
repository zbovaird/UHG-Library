...
class HyperbolicSGD(Optimizer):
    ...
    def update_parameter(
        self, p: torch.Tensor, group: Dict[str, Any], state: Dict[str, Any]
    ) -> torch.Tensor:
        ...
        if group["weight_decay"] != 0:
            grad = self.uhg.mobius_add(
                grad, self.uhg.mobius_scalar_mul(group["weight_decay"], p)
            )
        ...
        if group["momentum"] > 0:
            ...
            buf_new = self.uhg.mobius_add(
                self.uhg.mobius_scalar_mul(group["momentum"], buf), grad
            )
            ...
            if group["nesterov"]:
                ...
                grad = self.uhg.mobius_add(
                    grad, self.uhg.mobius_scalar_mul(group["momentum"], buf_new)
                )
            else:
                grad = buf_new

        # Apply update in hyperbolic space
        p_new = self.uhg.mobius_add(p, self.uhg.mobius_scalar_mul(-group["lr"], grad))
