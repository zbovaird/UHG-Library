...
class ProjectiveHMC(ProjectiveSampler):
    ...
    def sample(
        self, n_samples: int, initial_point: Optional[torch.Tensor] = None, **kwargs
    ) -> torch.Tensor:
        ...
        for _ in range(n_samples):
            ...
            for _ in range(self.n_steps):
                ...
                proposed_p = self.uhg.transform(
                    proposed_p, torch.eye(3) + self.step_size / 2 * force.unsqueeze(-1)
                )

                # Full step in position
                proposed_x = self.uhg.transform(
                    proposed_x, torch.eye(3) + self.step_size * proposed_p.unsqueeze(-1)
                )
            ...
            proposed_energy = (
                ...
                + self.uhg.cross_ratio(proposed_x, proposed_p, proposed_x, proposed_p)
                / 2
            )
